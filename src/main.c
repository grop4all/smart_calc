...
bool shunting_yard(const char *input, char *output) {
  ...
  while (strpos < strend) {
    c = *strpos;
    if (c != ' ') {
      // Если токен является числом (идентификатором), то добавить его в очередь
      // вывода.
      if (is_ident(c)) {
        *outpos = c;
        ++outpos;
      ...
      // Если токен - разделитель аргументов функции (запятая):
        ...
        // Если не была достигнута левая круглая скобка, либо разделитель не в
        // том месте либо была пропущена скобка
        if (!pe) {
      ...
      // Если токен оператор op1, то:
        ...
        while (sl > 0) {
          ...
          // а также оператор op1 лево-ассоциативный и его приоритет меньше или
          // такой же чем у оператора op2, или оператор op1 право-ассоциативный
      ...
      // Если токен - правая круглая скобка:
        ...
        bool pe = false;
        // До появления на вершине
        // стека токена "левая круглая скобка"
        // перекладывать операторы из стека в очередь вывода.
        while (sl > 0) {
          sc = stack[sl - 1];
          if (sc == '(') {
        ...
        // выкидываем токен "левая круглая скобка" из стека (не добавляем в
        // очередь вывода).
        sl--;
...
bool execution_order(const char *input) {
  ...
  while (strpos < strend) {
    ...
    // В противном случае, токен - оператор (здесь под оператором понимается как
    // оператор, так и название функции)
